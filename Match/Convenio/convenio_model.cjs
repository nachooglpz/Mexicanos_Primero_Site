...
const getConveniosAliado = async (usuario_aliado) => {
    const convenios = await db.query('select c.* from convenio c join usuarios_por_convenio upc on c.id_convenio = upc.id_convenio where upc.usuario_aliado = $1', [usuario_aliado]);
    return convenios.rows;
}

const getConveniosEscuela = async (usuario_escuela) => {
    const convenios = await db.query('select c.* from convenio c join usuarios_por_convenio upc on c.id_convenio = upc.id_convenio where upc.usuario_escuela = $1', [usuario_escuela]);
    return convenios.rows;
}

const getAllConvenios = async () => {
    const convenios = await db.query('SELECT * FROM convenio');
    return convenios.rows;
}

    ...
    useEffect(() => {
            if (usertype === 'aliado'){
                fetch(`/api/convenios/aliado?usuario=${username}`)
                    .then((res) => res.json())
                    .then((data) => {
                        const convenio = new Convenio(data.id_convenio, data.link_chat, data.link_contrato, data.estatus_firma_aliado, data.estatus_firma_escuela, data.finalizado, data.fecha_inicio);
                        setConvenios((prevConvenios) => [...prevConvenios, convenio]);
                    });

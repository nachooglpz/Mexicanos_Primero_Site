...
const getNoficicacionesByAdmin = async (admin) => {
    const notis = await db.query('SELECT n.* FROM notificaciones n JOIN notificaciones_a_administradores naa ON naa.id_notificacion = n.id_notificacion WHERE naa.usuario_admin = $1 OR naa.usuario_admin = $2', [admin, 'superadmin']);
    return notis.rows;
}
